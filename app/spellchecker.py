...
class SpellChecker:
    ...
    def _load_symspell_for_lang(self, lang: str):
        ...
        if not os.path.exists(freq_path):
            # no dictionary for this language — mark as None (no-op)
            self._symspell_map[lang] = None
            return None
        sym = SymSpell(max_dictionary_edit_distance=self.max_edit_distance, prefix_length=self.prefix_length)
        sym.create_dictionary(freq_path)
    ...
    def correct_text(self, text: str, lang: str = DEFAULT_LANG, max_results=2) -> str:
        ...
        suggestions = sym.lookup_compound(text, max_edit_distance=self.max_edit_distance)
        if suggestions:
